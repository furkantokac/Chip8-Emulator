...
{
    pc = 0x0200;// Chip8 game memory starts from 0x200 (512)
    opcode = 0; // Current code to run
    I = 0;      // General purpose register
    sp = 0;     // Stack pointer
    delay_timer = 0; // Initialize delay timer
    sound_timer = 0; // Initialize sound timer
    ...
    // Fontset holds the first 80 byte of the memory
    for (int i = 0; i < 80; i++)
        memory[i] = tmp_fontset[i];
...
bool Chip8::loadGame(QString path)
{
    ...
    {
        qDebug() << "ERROR! File is not found. Try again." << endl;
        return false;
    }
            ...
            for (int xline = 0; xline < 8; xline++)
            {
                ...
                if( syncgraphic )
                    if(gfx[x + xline + ((y + yline) * 64)])
                        gfxState[x + xline + ((y + yline) * 64)]++;

            }
        }
        if( syncgraphic && syncval>5 )
        {
            for (int i = 0; i < (64 * 32); i++)
            {
                if( gfxState[i]>0 )
